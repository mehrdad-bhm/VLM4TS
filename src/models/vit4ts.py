...
class ViT4TS:
    ...
    def predict_scores(self, data: pd.DataFrame) -> tuple[np.ndarray, np.ndarray]:
        # -> tuple[np.ndarray, np.ndarray] is only a type hint for  humans and tools(linters, IDEs)
        # You can ignore writing it if you want.
        """
        ...
        with tempfile.TemporaryDirectory() as temp_dir: 
            # This context manager automatically cleans up the directory after use
            results_dir = temp_dir
            base_series_id = "series"

            # 4. Generate windowed images
            step_size = int(self.window_size / self.window_step_ratio) # 224 / 4 = 56
            # 1D window that slides along the time axis, with a step size of 56 time points (for 75% overlap)
            
            '''
            Point:  0   1   2   3   4   5  ... 1000
            Value: 1.2 1.5 1.3 2.1 1.8 1.9 ... 2.0
                   |----window 224 points----| (0 ... 224)
                     |----step 56 points----|  (56 ... 280)
                          |----window 224 points----|
            '''
